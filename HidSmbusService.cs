    ...
    public class HidSmbusService
    {
        ...
        public byte[] BlockRead(byte deviceAddress, byte commandCode)
        {
            lock (_lock)
            {
                if (!PerformWrite(deviceAddress, new[] { commandCode })) return null;
                System.Threading.Thread.Sleep(20);
                // 假设HID设备处理块读取协议，返回[长度, 数据...]
                return PerformRead(deviceAddress, 32 + 1); // 读取最多32字节数据 + 1字节长度
            }
        }
        ...
        private byte[] PerformRead(byte deviceAddress, int bytesToRead)
        {
            ...
            outputReport[0] = 0x00; // 报告ID
            outputReport[1] = 0x52; // 'R' for Read
            outputReport[2] = (byte)(deviceAddress << 1 | 0x01); // 地址 + 读标志
            ...
            try
            {
                ...
                // 预期格式: [状态, 长度, 数据...]
                if (inputReport.Length > 2 && inputReport[0] == 0x01) // 0x01 = 成功
                {
        ...
        private bool PerformWrite(byte deviceAddress, byte[] data)
        {
            ...
            outputReport[2] = (byte)(deviceAddress << 1); // 地址 + 写标志
            ...
            try
            {
                _stream.Write(outputReport);
                var inputReport = _stream.Read();
                return inputReport.Length > 0 && inputReport[0] == 0x01; // 0x01 = 成功
            }
            catch (TimeoutException) { /* Ignore */ }
            return false;
        }
