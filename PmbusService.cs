...
namespace PMBusHidTool
{
    ...
    public class PmbusService
    {
        ...
        #region Limit Read/Write Methods
        ...
        #region Manual Command Passthrough
        ...
        {
            // 为简化起见，此示例假定使用固定指数，这在限制设置中很常见。
            // 一个更健壮的实现会读取设备支持的格式。
            const int exponent = -2; // 示例：IOUT的指数通常为-2
            short mantissa = (short)Math.Round(value / Math.Pow(2, exponent));

            // 打包成Linear11格式（为简洁起见，此处未完全实现，假定直接写入尾数）
            // 完整的实现需要打包尾数和指数。
            // 对于许多设备，设置限制涉及以预定义格式写入一个简单的字。
            // 我们假设设备接受给定命令的简单尾数。
            ushort rawValue = (ushort)mantissa; // 这是一个简化处理。
